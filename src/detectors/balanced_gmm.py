"""Balanced adaptive multi-mode background detector inspired by GMM."""
...
def gmm_background_mask(
    ...
    change_floor = max(6, round(threshold * 0.5))
    mask = []
    ...
    for index, pixel in enumerate(current):
        ...
        for mean, sigma, _count in components:
            adaptive_radius = max(change_floor, SIGMA_MATCH * sigma)
            distance = abs(pixel - mean)
            nearest_distance = max(nearest_distance, distance - adaptive_radius)
            if distance <= adaptive_radius:
                matched = True

        mask.append((not matched) and nearest_distance > 0)
